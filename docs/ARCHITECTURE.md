...
Every `iparsd` subcommand shares root observability options. When `--otel-enabled` or `IPARS_OTEL_ENABLED=true` is set, `iparsd` configures OTLP HTTP/protobuf exporters for traces and tracing-backed logs. `--otel-endpoint`/`IPARS_OTEL_ENDPOINT` accepts the collector base URL, with `/v1/traces` and `/v1/logs` selected per signal. `--otel-service-name`/`IPARS_OTEL_SERVICE_NAME` overrides the default `iparsd-<component>` service name, and `--log-filter`/`IPARS_LOG_FILTER` controls both local formatted tracing output and exported telemetry.
