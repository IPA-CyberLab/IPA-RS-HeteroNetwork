...
- Agent restart: identity and WireGuard keys are read from disk; the agent can re-register with a join token, report heartbeat state, then pinned routes and current peer map are rehydrated through explicit backend application and refreshed by continuous peer-map polling.
