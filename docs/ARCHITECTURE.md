...
The bundled Docker Compose and Helm manifests use HTTP service URLs for control-plane, signal, relay admission, and agent APIs because `iparsd` serves plain HTTP directly. In Compose, the host-network agent reads its join token from the mounted Compose secret and connects to co-located services through loopback URLs after service healthchecks report readiness. Production deployments should terminate TLS at a reverse proxy, load balancer, or Kubernetes Ingress before exposing those APIs outside the private deployment network.
