...
The chart can also render optional Services for the agent HTTP API and colocated relay UDP/HTTP endpoints. These are disabled by default so private underlay deployments do not publish node APIs unless an operator explicitly selects a Service type and annotations. NodePort and LoadBalancer Service types additionally require an exposure acknowledgement value in the chart; `ipars k8s install` only sets that acknowledgement when `--allow-public-service-exposure` is supplied. LoadBalancer Services require source ranges for agent and relay exposure unless the operator also sets the explicit unrestricted LoadBalancer acknowledgement. The install plan emits namespace creation, join-token Secret wiring, Helm upgrade/install flags, optional Service exposure toggles, and Service type/source-range/traffic-policy/annotation overrides. Relay exposure in that plan requires the public UDP endpoint and HTTP admission URL that should be advertised to peers.
