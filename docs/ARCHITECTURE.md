...
Every `iparsd` subcommand shares root observability options. When `--otel-enabled` or `IPARS_OTEL_ENABLED=true` is set, `iparsd` configures OTLP HTTP/protobuf exporters for traces, tracing-backed logs, and metrics. `--otel-endpoint`/`IPARS_OTEL_ENDPOINT` accepts the collector base URL, with `/v1/traces`, `/v1/logs`, and `/v1/metrics` selected per signal. `--otel-service-name`/`IPARS_OTEL_SERVICE_NAME` overrides the default `iparsd-<component>` service name, and `--log-filter`/`IPARS_LOG_FILTER` controls both local formatted tracing output and exported telemetry. The control-plane daemon records node, relay-candidate, health, path, and per-path-state gauges into `ipars.control_plane.*` OTLP metrics. The relay daemon records active/capacity gauges and dataplane counter deltas into `ipars.relay.*` OTLP metrics; the agent records candidate/path/relay-session gauges, per-path-state gauges, and relay-forwarder counter deltas into `ipars.agent.*` OTLP metrics. `--otel-metrics-poll-interval-seconds`/`IPARS_OTEL_METRICS_POLL_INTERVAL_SECONDS` controls control-plane, relay, and agent snapshot polling.
