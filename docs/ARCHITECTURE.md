...
During registration, the control plane validates that the submitted identity public key is valid Ed25519 key material and rejects endpoint candidates or advertised routes that belong to a different node ID. After registration, agents sign heartbeat updates with their node identity key. The control plane verifies the signature against the registered node identity public key, rejects missing, invalid, or stale heartbeat signatures, and only accepts endpoint candidates or path-state updates that belong to the reporting node. This keeps health, relay-capacity, candidate, route, and pair-scoped path-state updates bound to the node that owns the registered identity.
