...
- Control-plane HTTP crate with typed health, join, policy inspection, ACL-filtered peer-map, and JSON/Prometheus metrics routes backed by the join service.
- Control-plane peer-map and relay-map visibility filtering through cluster ACL rules matching source/target role, tag, route CIDR containment, and protocol, with deny rules taking precedence and default allow-all behavior when no rules are configured.
- `iparsd control-plane` daemon that serves the control-plane HTTP router with in-memory, SQLite, or PostgreSQL storage, accepts extra trusted issuer public keys for token-signing key rotation overlap, loads repeated JSON `--acl-rule` values or semicolon-separated `IPARS_ACL_RULES`, and exposes relay health TTL configuration.
- Signal registry, typed signal HTTP routes, JSON/Prometheus metrics, and `iparsd signal` for endpoint candidate exchange, relay candidate lookup, path negotiation, and hole-punch planning.
...
- `iparsd agent` signal path negotiation loop that fetches peer maps with ordered control-plane endpoint failover, calls `/v1/paths/negotiate` only for active or pinned peers, stores pair-scoped `PathRecord`s, removes idle relay runtime state, and includes negotiated path state in heartbeat reports.
- Signal node registration and path negotiation carry optional NAT classification and health data, so signal selection avoids `DIRECT_NAT_TRAVERSAL` when either side has classified itself as relay-preferred or insufficient for hole punching, and offers relay candidates only when they have fresh healthy signal health inside the configured relay health TTL.
...
- Linux command runtime preflight for `iparsd agent` that validates interface names, required `ip`/`wg` commands, `CAP_NET_ADMIN` when host networking will be mutated, `CAP_SYS_ADMIN` when Linux namespace placement is requested, and requested `/var/run/netns` namespace entries before starting data-plane application loops, rejecting missing entries, symlinks, and directories while warning on current-namespace placement.
...
- Packet-flow detector hardening beyond current typed ingestion, procfs conntrack polling, conntrack netlink table dumping, and conntrack NEW/UPDATE event subscription, including eBPF-based flow observation and richer flow metadata.
...
- Linux namespace lifecycle/capability hardening beyond current `CAP_SYS_ADMIN` startup preflight, `/var/run/netns` entry validation, and current-namespace placement warnings around command and netlink dataplane backends.
