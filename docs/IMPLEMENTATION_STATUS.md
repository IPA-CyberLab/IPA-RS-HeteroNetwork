...
- Persistent agent node state, STUN candidate collection, NAT classification status, agent status/path/STUN/NAT/peer-activity/packet-flow HTTP routes with optional source/protocol/port/detector metadata, and `iparsd agent`.
...
- `iparsd agent` signal-service node registration that upserts the registered `NodeRecord` with refreshed endpoint candidates across all known signal endpoints.
