...
- Persistent agent node state, STUN candidate collection, NAT classification status, agent status/path/STUN/NAT/peer-activity HTTP routes, and `iparsd agent`.
...
- Lazy connect and pinning runtime support in the agent crate and daemon loops, including peer activity recording, policy/route/relay/path pins, active-only signal negotiation, and idle peer removal during peer-map application.
...
- Packet-flow-driven lazy-connect activation beyond the current typed peer-activity API and pinned peer-map application.
