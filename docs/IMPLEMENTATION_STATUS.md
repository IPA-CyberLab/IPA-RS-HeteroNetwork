...
- CLI `init` and `token create` can sign join tokens with a reusable issuer private key and key ID; `join <token>` creates node identity/WireGuard keys, builds `JoinNodeRequest`, and posts to the token's control-plane bootstrap endpoint; `token revoke` posts typed token revocation requests to the control plane; `status`, `peers`, `routes`, `relay status`, and `path status` can query the agent, control-plane, and relay HTTP APIs when URLs are provided.
