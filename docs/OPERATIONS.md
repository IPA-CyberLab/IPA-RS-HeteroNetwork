...
The control plane accepts only fresh Ed25519-signed revocations from its configured issuer key ring. Keep overlapping old/new issuer public keys configured until tokens from the old key no longer need revocation. Redundant Control Planes initialize unseen ledger rows with insert-if-absent semantics and consume uses through compare-and-swap; SQL revocation changes only `revoked_at`. Concurrent first joins therefore cannot reset a counter, and a revocation does not erase uses that committed before it.
