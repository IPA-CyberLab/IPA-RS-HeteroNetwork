...
pub mod api {
    ...
    fn cassandra_request_body(
        ...
        match opcode {
            ...
            0x09 => cassandra_prepare_body(version, body_len, body_prefix),
    ...
    fn cassandra_response_body(
        ...
        match opcode {
            ...
            0x08 => cassandra_result_body(version, body_len, body_prefix),
    ...
    fn cassandra_result_body(version: u8, body_len: usize, body_prefix: &[u8]) -> bool {
        ...
        match kind {
            0x0001 => body_len == 4,
            0x0002 => cassandra_rows_result_body(version, body_len, body_prefix),
            0x0003 => cassandra_set_keyspace_result_body(body_len, body_prefix),
            0x0004 => cassandra_prepared_result_body(version, body_len, body_prefix),
            0x0005 => cassandra_schema_change_result_body(body_len, body_prefix),
            _ => false,
        }
    }

    fn cassandra_rows_result_body(version: u8, body_len: usize, body_prefix: &[u8]) -> bool {
        let Some((metadata_offset, columns_count)) =
            cassandra_result_metadata(version, body_prefix, 4, body_len)
        else {
            return false;
        };
        let Some(rows_count) =
    ...
    fn cassandra_prepared_result_body(version: u8, body_len: usize, body_prefix: &[u8]) -> bool {
        let Some(body) = body_prefix.get(..body_len) else {
            return false;
        };
        let Some(mut offset) = cassandra_short_bytes_field(body, 4, body_len) else {
            return false;
        };
        if version == 5 {
            let Some(next_offset) = cassandra_short_bytes_field(body, offset, body_len) else {
                return false;
            };
            offset = next_offset;
        }
        let Some((offset, _parameters_count)) = cassandra_prepared_metadata(body, offset, body_len)
        else {
            return false;
        };
        let Some((offset, _result_columns_count)) =
            cassandra_result_metadata(version, body, offset, body_len)
        else {
            return false;
        };
        offset == body_len
    ...
    fn cassandra_prepared_metadata(
        body_prefix: &[u8],
        offset: usize,
        body_len: usize,
    ) -> Option<(usize, usize)> {
        let header_end = offset.checked_add(12)?;
        if header_end > body_len {
            return None;
        }
        let flags = read_u32_be(body_prefix, offset)?;
        let columns_count = read_u32_be(body_prefix, offset.checked_add(4)?)? as usize;
        let pk_count = read_u32_be(body_prefix, offset.checked_add(8)?)? as usize;
        if flags & !0x0001 != 0 || columns_count > 4096 || pk_count > columns_count {
            return None;
        }

        let mut offset = header_end;
        let mut seen_pk_indices = vec![false; columns_count];
        for _ in 0..pk_count {
            let pk_index = read_u16_be(body_prefix, offset)? as usize;
            if pk_index >= columns_count || seen_pk_indices[pk_index] {
                return None;
            }
            seen_pk_indices[pk_index] = true;
            offset = offset.checked_add(2)?;
        }

        let global_spec = if flags & 0x0001 != 0 {
    ...
    fn cassandra_result_metadata(
        ...
        if metadata_changed {
            offset = cassandra_short_bytes_field(body_prefix, offset, body_len)?;
        }
        if no_metadata {
    ...
    fn cassandra_prepare_body(version: u8, body_len: usize, body_prefix: &[u8]) -> bool {
        if body_len < 4 || body_prefix.len() < 4 {
            return false;
        }
        let Some((query, offset)) = cassandra_long_string_field(body_prefix, 0, body_len) else {
            return false;
        };
        if !cassandra_cql_statement(trim_ascii_space(query)) {
            return false;
        }
        match version {
            3 | 4 => offset == body_len,
            5 => cassandra_prepare_v5_tail(body_prefix, offset, body_len),
            _ => false,
        }
    }

    fn cassandra_prepare_v5_tail(body_prefix: &[u8], mut offset: usize, body_len: usize) -> bool {
        let Some(flags_end) = offset.checked_add(4) else {
            return false;
        };
        if flags_end > body_len || flags_end > body_prefix.len() {
            return false;
        }
        let Some(flags) = read_u32_be(body_prefix, offset) else {
            return false;
        };
        if flags & !0x01 != 0 {
            return false;
    ...
    fn cassandra_execute_body(version: u8, body_len: usize, body_prefix: &[u8]) -> bool {
        ...
        let Some(prepared_id_end) = cassandra_short_bytes_field(body_prefix, 0, body_len) else {
            return false;
        };
        let parameters_offset = if version == 5 {
            let Some(result_metadata_id_end) =
                cassandra_short_bytes_field(body_prefix, prepared_id_end, body_len)
            else {
                return false;
            };
            result_metadata_id_end
        } else {
            prepared_id_end
        };
        let Some(parameter_header_len) = cassandra_query_parameter_header_len(version) else {
            return false;
        };
        let Some(required_len) = parameters_offset.checked_add(parameter_header_len) else {
            return false;
        };
        body_len >= required_len
            && body_prefix.len() >= required_len
            && cassandra_query_parameters(version, body_len, body_prefix, parameters_offset)
    }
...
mod tests {
    ...
    fn packet_flow_observation_classifies_payload_prefix_application_protocol() {
        ...
        fn cassandra_short_bytes(value: &[u8]) -> Vec<u8> {
            let mut encoded = (value.len() as u16).to_be_bytes().to_vec();
            encoded.extend_from_slice(value);
            encoded
        }
        ...
        fn cassandra_prepared_metadata_body(
            ...
            encoded.extend_from_slice(&(pk_indices.len() as u32).to_be_bytes());
            for pk_index in pk_indices {
                encoded.extend_from_slice(&pk_index.to_be_bytes());
            }
            if let Some((keyspace, table)) = global_table {
                encoded.extend_from_slice(&cassandra_string(keyspace));
                encoded.extend_from_slice(&cassandra_string(table));
            }
            for (keyspace, table, name, type_id) in columns {
                if global_table.is_none() {
                    encoded.extend_from_slice(&cassandra_string(keyspace));
                    encoded.extend_from_slice(&cassandra_string(table));
                }
                encoded.extend_from_slice(&cassandra_string(name));
                encoded.extend_from_slice(&type_id.to_be_bytes());
            }
            encoded
        }

        fn cassandra_result_metadata_body(
            flags: u32,
            ...
            encoded.extend_from_slice(&(columns.len() as u32).to_be_bytes());
            if flags & 0x0008 != 0 {
                encoded.extend_from_slice(&cassandra_short_bytes(metadata_id.unwrap_or(b"mid")));
            }
            if flags & 0x0004 == 0 {
                if let Some((keyspace, table)) = global_table {
                    encoded.extend_from_slice(&cassandra_string(keyspace));
        ...
        fn cassandra_prepare_frame(
            ...
            if version == 5 {
                ...
                if let Some(keyspace) = keyspace {
                    body.extend_from_slice(&cassandra_string(keyspace));
                }
            }
            cassandra_request_frame_with_flags(version, 0, 0x09, &body)
        }

        fn cassandra_execute_frame(
            version: u8,
            prepared_id: &[u8],
            result_metadata_id: Option<&[u8]>,
            consistency: u16,
            flags: u32,
            tail: &[u8],
        ) -> Vec<u8> {
            let mut body = Vec::new();
            body.extend_from_slice(&(prepared_id.len() as u16).to_be_bytes());
            body.extend_from_slice(prepared_id);
            if version == 5 {
                let result_metadata_id = result_metadata_id.unwrap_or(b"rid");
                body.extend_from_slice(&(result_metadata_id.len() as u16).to_be_bytes());
                body.extend_from_slice(result_metadata_id);
            }
            body.extend_from_slice(&consistency.to_be_bytes());
            if version == 5 {
                body.extend_from_slice(&flags.to_be_bytes());
            ...
            body.extend_from_slice(tail);
            cassandra_request_frame_with_flags(version, 0, 0x0a, &body)
        }
        ...
        assert_eq!(
            observation_for_payload(&cassandra_prepare_frame(0x04, b"SELECT 1", 0, None))
                .application(),
            api::AgentPacketFlowApplication::Cassandra
        );
        assert_eq!(
            observation_for_payload(&cassandra_prepare_frame(
                0x05,
                b"SELECT * FROM tbl WHERE id=?",
                0x01,
                Some(b"ks")
            ))
            .application(),
        ...
        assert_eq!(
            observation_for_payload(&cassandra_execute_frame(0x04, b"pid", None, 0x0001, 0, &[]))
                .application(),
            api::AgentPacketFlowApplication::Cassandra
        );
        assert_eq!(
            observation_for_payload(&cassandra_execute_frame(
                0x05,
                b"pid",
                Some(b"rid"),
                0x0001,
        ...
        let mut cassandra_execute_v5_tail = Vec::new();
        cassandra_execute_v5_tail.extend_from_slice(&1_u16.to_be_bytes());
        cassandra_execute_v5_tail
            .extend_from_slice(&cassandra_bytes_value(Some(&1234_i32.to_be_bytes())));
        cassandra_execute_v5_tail.extend_from_slice(&cassandra_string(b"ks"));
        cassandra_execute_v5_tail.extend_from_slice(&42_u32.to_be_bytes());
        assert_eq!(
            observation_for_payload(&cassandra_execute_frame(
                0x05,
                b"pid",
        ...
        let mut cassandra_rows_v5_metadata_changed = 2_u32.to_be_bytes().to_vec();
        cassandra_rows_v5_metadata_changed.extend_from_slice(&cassandra_result_metadata_body(
            0x0009,
            Some(b"mid"),
            Some((b"ks", b"tbl")),
            &[(b"ks", b"tbl", b"id", 0x0009)],
        ));
        cassandra_rows_v5_metadata_changed.extend_from_slice(&0_u32.to_be_bytes());
        assert_eq!(
            observation_for_payload(&cassandra_response_frame_with_flags(
                0x85,
                0,
                0x08,
                &cassandra_rows_v5_metadata_changed
            ))
            .application(),
            api::AgentPacketFlowApplication::Cassandra
        );
        let mut cassandra_prepared_result = 4_u32.to_be_bytes().to_vec();
        cassandra_prepared_result.extend_from_slice(&cassandra_short_bytes(b"pid"));
        cassandra_prepared_result.extend_from_slice(&cassandra_prepared_metadata_body(
            0x0001,
            &[0],
            Some((b"ks", b"tbl")),
            &[(b"ks", b"tbl", b"id", 0x0009)],
        ));
        cassandra_prepared_result.extend_from_slice(&cassandra_result_metadata_body(
            0x0004,
            None,
            None,
            &[],
        ...
        let mut cassandra_prepared_result_v5 = 4_u32.to_be_bytes().to_vec();
        cassandra_prepared_result_v5.extend_from_slice(&cassandra_short_bytes(b"pid"));
        cassandra_prepared_result_v5.extend_from_slice(&cassandra_short_bytes(b"rid"));
        cassandra_prepared_result_v5.extend_from_slice(&cassandra_prepared_metadata_body(
            0x0001,
            &[0],
            Some((b"ks", b"tbl")),
            &[(b"ks", b"tbl", b"id", 0x0009)],
        ));
        cassandra_prepared_result_v5.extend_from_slice(&cassandra_result_metadata_body(
            0x0004,
            None,
            None,
            &[],
        ...
        let mut cassandra_v5_prepare_missing_flags = Vec::new();
        cassandra_v5_prepare_missing_flags.extend_from_slice(&8_u32.to_be_bytes());
        cassandra_v5_prepare_missing_flags.extend_from_slice(b"SELECT 1");
        assert_eq!(
            observation_for_payload(&cassandra_request_frame_with_flags(
                0x05,
                0,
                0x09,
                &cassandra_v5_prepare_missing_flags
            ))
            .application(),
            api::AgentPacketFlowApplication::Unknown
        );
        assert_eq!(
            observation_for_payload(&cassandra_prepare_frame(0x05, b"SELECT 1", 0x02, None))
                .application(),
            api::AgentPacketFlowApplication::Unknown
        );
        assert_eq!(
            observation_for_payload(&cassandra_prepare_frame(0x05, b"SELECT 1", 0x01, None))
                .application(),
            api::AgentPacketFlowApplication::Unknown
        );
        assert_eq!(
            observation_for_payload(&cassandra_prepare_frame(0x05, b"SELECT 1", 0, Some(b"ks")))
                .application(),
            api::AgentPacketFlowApplication::Unknown
        );
        let mut cassandra_v4_prepare_with_v5_tail = Vec::new();
        cassandra_v4_prepare_with_v5_tail.extend_from_slice(&8_u32.to_be_bytes());
        cassandra_v4_prepare_with_v5_tail.extend_from_slice(b"SELECT 1");
        cassandra_v4_prepare_with_v5_tail.extend_from_slice(&1_u32.to_be_bytes());
        cassandra_v4_prepare_with_v5_tail.extend_from_slice(&cassandra_string(b"ks"));
        assert_eq!(
            observation_for_payload(&cassandra_request_frame_with_flags(
                0x04,
                0,
                0x09,
                &cassandra_v4_prepare_with_v5_tail
            ))
            .application(),
            api::AgentPacketFlowApplication::Unknown
        );
        let mut cassandra_v5_execute_missing_metadata_id = Vec::new();
        cassandra_v5_execute_missing_metadata_id.extend_from_slice(&3_u16.to_be_bytes());
        cassandra_v5_execute_missing_metadata_id.extend_from_slice(b"pid");
        cassandra_v5_execute_missing_metadata_id.extend_from_slice(&0x0001_u16.to_be_bytes());
        cassandra_v5_execute_missing_metadata_id.extend_from_slice(&0_u32.to_be_bytes());
        assert_eq!(
            observation_for_payload(&cassandra_request_frame_with_flags(
                0x05,
                0,
                0x0a,
                &cassandra_v5_execute_missing_metadata_id
            ))
            .application(),
            api::AgentPacketFlowApplication::Unknown
        );
        ...
        let mut cassandra_rows_v4_metadata_changed = 2_u32.to_be_bytes().to_vec();
        cassandra_rows_v4_metadata_changed.extend_from_slice(&cassandra_result_metadata_body(
            0x0008,
            Some(b"mid"),
            Some((b"ks", b"tbl")),
            &[(b"ks", b"tbl", b"id", 0x0009)],
        ));
        cassandra_rows_v4_metadata_changed.extend_from_slice(&0_u32.to_be_bytes());
        assert_eq!(
            observation_for_payload(&cassandra_response_frame(
                0x08,
                &cassandra_rows_v4_metadata_changed
            ))
            .application(),
            api::AgentPacketFlowApplication::Unknown
        );
        let mut cassandra_prepared_legacy_metadata_shape = 4_u32.to_be_bytes().to_vec();
        cassandra_prepared_legacy_metadata_shape.extend_from_slice(&cassandra_short_bytes(b"pid"));
        cassandra_prepared_legacy_metadata_shape.extend_from_slice(&4_u32.to_be_bytes());
        cassandra_prepared_legacy_metadata_shape.extend_from_slice(&0_u32.to_be_bytes());
        cassandra_prepared_legacy_metadata_shape.extend_from_slice(&4_u32.to_be_bytes());
        cassandra_prepared_legacy_metadata_shape.extend_from_slice(&0_u32.to_be_bytes());
        assert_eq!(
            observation_for_payload(&cassandra_response_frame(
                0x08,
                &cassandra_prepared_legacy_metadata_shape
            ))
            .application(),
            api::AgentPacketFlowApplication::Unknown
        );
        let mut cassandra_v5_prepared_missing_result_metadata_id = 4_u32.to_be_bytes().to_vec();
        cassandra_v5_prepared_missing_result_metadata_id
            .extend_from_slice(&cassandra_short_bytes(b"pid"));
        cassandra_v5_prepared_missing_result_metadata_id.extend_from_slice(
            &cassandra_prepared_metadata_body(
                0x0001,
                &[0],
                Some((b"ks", b"tbl")),
                &[(b"ks", b"tbl", b"id", 0x0009)],
        ...
        cassandra_v5_prepared_missing_result_metadata_id
            .extend_from_slice(&cassandra_result_metadata_body(0x0004, None, None, &[]));
        assert_eq!(
            observation_for_payload(&cassandra_response_frame_with_flags(
                0x85,
                0,
        ...
        let mut cassandra_prepared_bad_pk_index = 4_u32.to_be_bytes().to_vec();
        cassandra_prepared_bad_pk_index.extend_from_slice(&cassandra_short_bytes(b"pid"));
        cassandra_prepared_bad_pk_index.extend_from_slice(&cassandra_prepared_metadata_body(
            0x0001,
            &[1],
            Some((b"ks", b"tbl")),
            &[(b"ks", b"tbl", b"id", 0x0009)],
        ));
        cassandra_prepared_bad_pk_index.extend_from_slice(&cassandra_result_metadata_body(
            0x0004,
            None,
            None,
            &[],
        ...
        assert_eq!(
            observation_for_payload(&cassandra_response_frame(
                0x08,
                &cassandra_prepared_bad_pk_index
            ))
            .application(),
            api::AgentPacketFlowApplication::Unknown
        );
