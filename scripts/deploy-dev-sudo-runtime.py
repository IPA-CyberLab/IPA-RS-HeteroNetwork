...
BUNDLE = Path("/opt/heteronetwork-dev-sudo-runtime-deploy-v3")
FILES = {
    "provision-dev-sudo-runtime.py": ("1baf64fd95a9a44f5e86f0bb8eb365ac60649ddb06e08355a975a4ba29eac821", 9478),
...
root=pathlib.Path("/opt/heteronetwork-dev-sudo-runtime-v3");path=root/name;digest,size=files[name]
...
def deploy(member, payloads):
    ...
    result = json.loads(invoke(member, ["sudo", "-n", "/usr/bin/python3", "-B",
                                        "/opt/heteronetwork-dev-sudo-runtime-v3/provision-dev-sudo-runtime.py"]))
