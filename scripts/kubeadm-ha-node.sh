...
readonly DEFAULT_AGENT_STATUS_URL="http://127.0.0.1:9780/v1/status"
...
Commands:
  ...
  reconcile-discovered-control-plane-backends
                         Discover the HA cohort from the local Agent and update the API proxy
  install-api-backend-autopilot
                         Install the periodic control-plane backend reconciler
...
render_api_backend_autopilot_service() {
  cat <<'EOF'
[Unit]
Description=Reconcile HeteroNetwork Kubernetes API backends
Wants=heteronetwork-agent.service
After=heteronetwork-agent.service
ConditionPathExists=/etc/heteronetwork/kubernetes/node.env
...
render_api_backend_autopilot_timer() {
  cat <<'EOF'
[Unit]
Description=Periodically reconcile HeteroNetwork Kubernetes API backends
...
secure_root_state_file() {
  local path="$1" maximum_size="$2" uid links mode size
  [[ "$path" == /* && -f "$path" && ! -L "$path" ]] || return 1
  read -r uid links mode size < <(stat -c '%u %h %a %s' -- "$path") \
    || return 1
  [[ "$uid" == 0 && "$links" == 1 && "$mode" == 600 ]] || return 1
  ((10#$size > 0 && 10#$size <= maximum_size))
}

load_local_node_state() {
  local node_state="$state_dir/node.env"
  secure_root_state_file "$node_state" 65536 \
  ...
  unset \
    ...
    HETERONETWORK_KUBEADM_CONTROL_PLANES \
    HETERONETWORK_KUBEADM_PREFERRED_CONTROL_PLANE \
    HETERONETWORK_KUBEADM_APISERVER_ETCD_ENDPOINTS \
    HETERONETWORK_KUBEADM_API_NAME \
    HETERONETWORK_KUBEADM_API_PROXY_PORT \
    HETERONETWORK_KUBEADM_POD_CIDR \
    HETERONETWORK_KUBEADM_SERVICE_CIDR \
    HETERONETWORK_KUBEADM_KUBERNETES_MINOR \
    HETERONETWORK_KUBEADM_MAX_PODS
  # node.env is generated by this helper and accepted only when root-owned 0600.
  ...
  preferred_control_plane="${HETERONETWORK_KUBEADM_PREFERRED_CONTROL_PLANE:-}"
  apiserver_etcd_backends="${HETERONETWORK_KUBEADM_APISERVER_ETCD_ENDPOINTS:-}"
  api_name="${HETERONETWORK_KUBEADM_API_NAME:-}"
  api_proxy_port="${HETERONETWORK_KUBEADM_API_PROXY_PORT:-}"
  pod_cidr="${HETERONETWORK_KUBEADM_POD_CIDR:-}"
  service_cidr="${HETERONETWORK_KUBEADM_SERVICE_CIDR:-}"
  kubernetes_minor="${HETERONETWORK_KUBEADM_KUBERNETES_MINOR:-}"
  max_pods="${HETERONETWORK_KUBEADM_MAX_PODS:-}"
  validate_common_config
}

select_discovered_control_plane_addresses() {
  local peers_file="$1" local_node_file="$2" configured_json="$3"
  local addresses_file count address
  addresses_file="$(mktemp)"
  if ! jq -ner \
    --arg generic_tag "$KUBERNETES_CONTROL_PLANE_TAG" \
    --arg cohort_prefix "$KUBERNETES_HA_TAG_PREFIX" \
    --argjson configured "$configured_json" \
    --argjson maximum "$MAX_DISCOVERED_CONTROL_PLANES" \
    --slurpfile peers "$peers_file" \
    --slurpfile local_node "$local_node_file" '
      def has_tag($tag):
        (.tags | type == "array") and any(.tags[]; . == $tag);
      ($peers[0].peers + [$local_node[0]]) as $nodes
      | [
          $nodes[]
          | select((.vpn_ip | type == "string") and has_tag($generic_tag))
          | . as $node
          | ($node.tags[]
              | select(type == "string" and startswith($cohort_prefix))) as $cohort
          | {
              cohort: $cohort,
              vpn_ip: $node.vpn_ip,
      ...
      | map({
          cohort: .[0].cohort,
          count: ([.[].vpn_ip] | unique | length),
          overlap: ([.[] | select(.configured) | .vpn_ip] | unique | length)
        })
      | map(select(
          .count >= 3
          and .count <= $maximum
          and (.count % 2 == 1)))
      | sort_by([-.overlap, -.count, .cohort])
      | (if length == 0 then
          error("no eligible Kubernetes HA cohort")
        elif length == 1 then
          .[0].cohort
        elif .[0].overlap >= 2 and .[0].overlap > .[1].overlap then
          .[0].cohort
        else
  ...
  if ((10#$count < 3 \
    || 10#$count > MAX_DISCOVERED_CONTROL_PLANES \
    || 10#$count % 2 == 0)); then
    rm -f "$addresses_file"
    return 1
  fi
  ...
  LC_ALL=C sort -V -u "$addresses_file" | paste -sd, -
  rm -f "$addresses_file"
}

discover_control_plane_addresses() (
  local status_file peers_file local_node_file configured_json selected
  status_file="$(mktemp)"
  peers_file="$(mktemp)"
  local_node_file="$(mktemp)"
  trap 'rm -f "$status_file" "$peers_file" "$local_node_file"' EXIT

  secure_root_state_file "$agent_state_path" 1048576 \
    || die "HeteroNetwork Agent state is missing or insecure"
  curl --fail --silent --show-error --connect-timeout 2 --max-time 5 \
    --max-filesize "$MAX_AGENT_DIRECTORY_BYTES" \
    "$agent_status_url" -o "$status_file"
  curl --fail --silent --show-error --connect-timeout 2 --max-time 8 \
    --max-filesize "$MAX_AGENT_DIRECTORY_BYTES" \
    "$agent_peers_url" -o "$peers_file"
  jq -e --arg node_ip "$node_ip" '
    type == "object"
    and (.vpn_ip == $node_ip)
    and (.node_id | type == "string" and length >= 1 and length <= 255)
  ' "$status_file" >/dev/null \
    || die "local Agent status does not match Kubernetes node state"
  jq -e --arg node_ip "$node_ip" '
    .registered_node
    | select(
        type == "object"
        and (.vpn_ip == $node_ip)
        and (.tags | type == "array")
        and all(.tags[]; type == "string"))
    | {vpn_ip, tags}
  ' "$agent_state_path" >"$local_node_file" \
    || die "local Agent enrollment does not match Kubernetes node state"
  jq -e '
    type == "object"
    ...
    and all(.peers[];
      ...
      and all(.tags[]; type == "string"))
  ' "$peers_file" >/dev/null \
    || die "Agent peer directory is invalid"

  configured_json="$(backend_addresses | jq -R . | jq -s .)"
  selected="$(select_discovered_control_plane_addresses \
    "$peers_file" "$local_node_file" "$configured_json")" \
    || die "unable to select one Kubernetes HA cohort from the Agent directory"
  printf '%s\n' "$selected"
)
...
configure_haproxy() {
  local config_temporary unit_temporary config_changed=0 unit_changed=0
  config_temporary="$(mktemp)"
  unit_temporary="$(mktemp)"
  render_haproxy_config >"$config_temporary"
  if ! /usr/sbin/haproxy -c -f "$config_temporary" >/dev/null; then
    rm -f "$config_temporary" "$unit_temporary"
    die "rendered Kubernetes API load balancer configuration is invalid"
  fi
  render_haproxy_service >"$unit_temporary"

  if [[ -f "$state_dir/haproxy.cfg" \
    && ! -L "$state_dir/haproxy.cfg" \
    && -f /etc/systemd/system/heteronetwork-kube-apiserver-lb.service \
    && ! -L /etc/systemd/system/heteronetwork-kube-apiserver-lb.service ]]; then
    if ! cmp -s "$config_temporary" "$state_dir/haproxy.cfg"; then
      install -D -o root -g root -m 0644 \
        "$config_temporary" "$state_dir/haproxy.cfg"
      config_changed=1
    fi
    if ! cmp -s "$unit_temporary" \
      /etc/systemd/system/heteronetwork-kube-apiserver-lb.service; then
      install -D -o root -g root -m 0644 "$unit_temporary" \
        /etc/systemd/system/heteronetwork-kube-apiserver-lb.service
      unit_changed=1
    fi
  else
    [[ ! -L "$state_dir/haproxy.cfg" \
      && ! -L /etc/systemd/system/heteronetwork-kube-apiserver-lb.service ]] \
      || {
        rm -f "$config_temporary" "$unit_temporary"
        die "refusing symlinked Kubernetes API load balancer state"
      }
    install -D -o root -g root -m 0644 \
      "$config_temporary" "$state_dir/haproxy.cfg"
    install -D -o root -g root -m 0644 "$unit_temporary" \
  ...
  systemctl enable heteronetwork-kube-apiserver-lb.service >/dev/null
  if systemctl is-active --quiet heteronetwork-kube-apiserver-lb.service; then
    if ((config_changed == 1 || unit_changed == 1)); then
      systemctl reload-or-restart heteronetwork-kube-apiserver-lb.service
    fi
  else
    systemctl start heteronetwork-kube-apiserver-lb.service
  fi
...
install_api_backend_autopilot() {
  ...
  render_api_backend_autopilot_timer >"$timer_temporary"

  [[ ! -L "$service_path" && ! -L "$timer_path" ]] || {
    rm -f "$service_temporary" "$timer_temporary"
    die "refusing symlinked Kubernetes API backend autopilot units"
  }
  if [[ ! -f "$service_path" ]] || ! cmp -s "$service_temporary" "$service_path"; then
    install -o root -g root -m 0644 "$service_temporary" "$service_path"
    changed=1
  fi
  if [[ ! -f "$timer_path" ]] || ! cmp -s "$timer_temporary" "$timer_path"; then
    install -o root -g root -m 0644 "$timer_temporary" "$timer_path"
    changed=1
  fi
  rm -f "$service_temporary" "$timer_temporary"
  if ((changed == 1)); then
  ...
  systemctl enable --now heteronetwork-kubeadm-backend-autopilot.timer >/dev/null
}
...
reconcile_discovered_control_plane_backends() {
  ...
  require_command jq
  require_command systemctl
  exec 9>/run/heteronetwork-kubeadm-backend-autopilot.lock
  flock -n 9 || return 0
...
install_api_backend_autopilot_command() {
  ...
  load_local_node_state
  install_api_backend_autopilot
  systemctl start --no-block heteronetwork-kubeadm-backend-autopilot.service
  printf 'Kubernetes API backend autopilot installed for %s\n' "$node_name"
}
...
self_test() {
  ...
  local discovery_peers discovery_local configured_json selected
  ...
  jq -n \
    --arg generic "$KUBERNETES_CONTROL_PLANE_TAG" \
    --arg selected_cohort "${KUBERNETES_HA_TAG_PREFIX}0123456789abcdef" \
    --arg other_cohort "${KUBERNETES_HA_TAG_PREFIX}fedcba9876543210" '{peers: [
      {vpn_ip: "10.250.0.1", tags: [$generic, $selected_cohort]},
      {vpn_ip: "10.250.0.3", tags: [$generic, $selected_cohort]},
      {vpn_ip: "10.250.0.4", tags: [$generic, $selected_cohort]},
      {vpn_ip: "10.250.0.5", tags: [$generic, $selected_cohort]},
      {vpn_ip: "10.250.0.20", tags: [$generic, $other_cohort]},
      {vpn_ip: "10.250.0.21", tags: [$generic, $other_cohort]},
      {vpn_ip: "10.250.0.22", tags: [$generic, $other_cohort]}
    ]}' >"$discovery_peers"
  jq -n \
    --arg generic "$KUBERNETES_CONTROL_PLANE_TAG" \
    --arg cohort "${KUBERNETES_HA_TAG_PREFIX}0123456789abcdef" \
    '{vpn_ip: "10.250.0.2", tags: [$generic, $cohort]}' >"$discovery_local"
  configured_json='["10.250.0.1","10.250.0.2","10.250.0.3"]'
  selected="$(select_discovered_control_plane_addresses \
    "$discovery_peers" "$discovery_local" "$configured_json")"
  [[ "$selected" == "10.250.0.1,10.250.0.2,10.250.0.3,10.250.0.4,10.250.0.5" ]] \
    || die "backend discovery did not expand the selected HA cohort"
  configured_json='["10.250.0.30","10.250.0.31","10.250.0.32"]'
  if select_discovered_control_plane_addresses \
    "$discovery_peers" "$discovery_local" "$configured_json" >/dev/null 2>&1; then
    die "backend discovery accepted ambiguous Kubernetes HA cohorts"
  fi
  rm -f "$discovery_peers" "$discovery_local"
  control_plane_backends="10.250.0.1,10.250.0.2,10.250.0.3"
  ...
  rendered="$(render_api_backend_autopilot_service)"
  grep -Fq \
    'ExecStart=/opt/heteronetwork/libexec/kubeadm-ha-node.sh reconcile-discovered-control-plane-backends' \
    <<<"$rendered"
  grep -Fq \
