...
const refreshCookiePath = "/ui/auth";
...
export function createWebConsoleServer({ refreshRuntime = {} } = {}) {
  ...
  return createServer(async (request, response) => {
    try {
      ...
      if (request.method === "POST" && url.pathname === "/ui/auth/refresh") {
        return await handleRefresh(request, response, refreshCoordinator);
      }
      if (request.method === "POST" && url.pathname === "/ui/auth/logout") {
...
async function handleCallback(response, url) {
  ...
  const tokens = validateOidcTokens(await boundedJson(tokenResponse));
...
sessionStorage.setItem("heteronetwork_access_token", ${safeJson(tokens.access_token)});
sessionStorage.setItem("heteronetwork_access_token_expires_at",String(Date.now()+${tokens.expires_in}*1000));
location.replace("/ui/");
</script>`);
}
...
function publicConfig() {
  return {
    ...
    session_refresh_endpoint: "/ui/auth/refresh",
    session_logout_endpoint: "/ui/auth/logout",
  };
}
